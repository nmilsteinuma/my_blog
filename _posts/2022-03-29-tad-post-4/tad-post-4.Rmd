...
topfeatures(new_guns_corpus_dfm_tl, 20) %>% kable()

```

**Explanation:** Getting rid of punctuation helps clarify some of the features and gets rid of many of the pieces of punctuation such as the period and question.

```{r}

topfeatures(new_guns_corpus_dfm_punct_tl_sw, 20) %>% kable()
...
mean(new_guns_corpus_summary$Types) %>% kable()

quantile(new_guns_corpus_summary$Types) %>% kable()
...
mean(new_guns_corpus_summary$Tokens) %>% kable() 

quantile(new_guns_corpus_summary$Tokens) %>% kable()
...
mean(new_guns_corpus_summary$Sentences) %>% kable()

quantile(new_guns_corpus_summary$Sentences) %>% kable()
...
** Basis:** The trend is largely the same as stop words with many more observations of the most frequently occurring words.
...
# More than 10% and 5% of documents and 2.5% of documents
...
**Basis:** The crux of this project will be a topic modeling analysis of the top reddit posts and a comparison with the topics in advertisements from *Targeted advertising: documenting the emergence of Gun Culture 2.0 in Guns magazine, 1955–2019* in this paper the author distinguishes 4 broad categories of advertised use, Hunting, Sport/Recreation, Personal protection/self-defense/home or family
defense, and finally Concealed carry. I will be using topic modeling to see if it is possible to distinguish posts into these use categories and compare to the advertising results.
...
top_guns_urls_df <- loadRData("/Users/noahmilstein/Desktop/Spring 2022/Textasdata/text_as_data_work/top_guns_urls.RData")
...
top_guns_urls_df<-top_guns_urls_df[,c("title", "date_utc", "comments")]

new_guns_corpus<-corpus(top_guns_urls_df$title)
...
readability_new_guns$added_dates<- as.Date(top_guns_urls_df$date_utc)
...
new_guns_urls_df_2<-top_guns_urls_df
...
## Last Scrape Hot Posts and combining all data.

```{r}
#hot_guns_urls <- find_thread_urls(subreddit="guns", sort_by="hot")
# save(hot_guns_urls, file="hot_guns_urls.RData")
hot_guns_urls<-loadRData("/Users/noahmilstein/Desktop/my_blog/_posts/2022-03-29-tad-post-4/hot_guns_urls.RData")

hot_guns_urls_df<-hot_guns_urls[,c("title", "date_utc", "comments")]
#hot_guns_urls_df
