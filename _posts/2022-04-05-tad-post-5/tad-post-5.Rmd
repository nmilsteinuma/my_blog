...
lda_model <- LDA$new(n_topics = 20, doc_topic_prior = 0.1, 
...
processedCorpus_guns <- data.frame(doc_id=row.names(whole_data_frame_unique),
                       text=whole_data_frame_unique$all_text)

corpus <- Corpus(DataframeSource(df_all_guns_for_corpus))

# Preprocessing chain

processedCorpus_guns <- tm_map(corpus, content_transformer(tolower))
processedCorpus_guns <- tm_map(processedCorpus, removeWords, english_stopwords)
...
ggplot(df_for_visualization, aes(x=group_names, y=countsOfPrimaryTopics,  
  ...
  geom_bar(stat = "identity" )+  theme_classic() +
   theme(axis.text.x = element_text(angle = 90, hjust = 1, size=4),legend.key.size = unit(0.3, 'cm'), #change legend key size
